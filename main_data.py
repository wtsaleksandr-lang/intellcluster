...
@media(max-width:700px){body[data-intell-search] .mini-stat{padding:0 8px!important;border-right:0!important}body[data-intell-search] .stats-row{gap:10px!important}body[data-intell-search] .ic-help::after{left:auto;right:-8px;transform:translateY(5px);width:210px}body[data-intell-search] .ic-help:hover::after,body[data-intell-search] .ic-help:focus::after{transform:translateY(0)}}
...
(()=>{
  ...
  if(searchPage){
    ...
    document.querySelectorAll('.card').forEach(card=>{
      ...
      if(address){
        const nodes=[...address.querySelectorAll('span')];
        const countryNode=nodes.find(n=>countryCodes[(n.textContent||'').trim()]);
        if(countryNode&&!countryNode.querySelector('.ic-country-flag')){
          const f=document.createElement('span');f.className='ic-country-flag';f.textContent=flag(countryNode.textContent);f.setAttribute('aria-hidden','true');countryNode.prepend(f);
        }
      }
      card.querySelectorAll('a[href*="/data/origin/"]').forEach(a=>{
        const label=(a.textContent||'').trim();
        const f=flag(label);
        if(f&&!a.querySelector('.ic-country-flag')){
          a.querySelector('svg')?.remove();
          const span=document.createElement('span');span.className='ic-country-flag';span.textContent=f;span.setAttribute('aria-hidden','true');a.prepend(span);
        }
      });
      const helpText={
        'AI Buyer Score':'Internal ranking signal that helps prioritize potentially relevant buyers. It is a decision aid, not a credit or financial score.',
        'HS Codes':'Number of distinct Harmonized System product codes connected to this company in the indexed public trade records.',
        'Source Countries':'Number of distinct origin markets connected to this company in the indexed importer records.',
        'Matched Sources':'Number of independent IntellCluster datasets currently linked to this canonical company profile.'
      };
      card.querySelectorAll('.mini-label').forEach(label=>{
        const key=(label.textContent||'').trim();
        const copy=helpText[key];
        if(!copy||label.querySelector('.ic-help'))return;
        const help=document.createElement('span');help.className='ic-help';help.tabIndex=0;help.textContent='?';help.dataset.help=copy;help.setAttribute('aria-label',`${key}: ${copy}`);label.appendChild(help);
      });
    });
    const meta=document.querySelector('.result-meta .hint');
    if(meta){meta.innerHTML='<span class="ic-coverage-help">Clickable intelligence facets <span class="ic-help" tabindex="0" data-help="HS codes, sourcing countries and locations open dedicated analytical drill-down pages instead of starting a new generic search.">?</span></span>';}
    return;
  }
