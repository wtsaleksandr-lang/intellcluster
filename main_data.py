...
body[data-intell-profile] .ic-us-public{margin:18px 0;border:1px solid #d5e0e6;border-radius:8px;background:#fff;overflow:hidden}.ic-us-public-head{display:flex;align-items:center;justify-content:space-between;gap:14px;padding:13px 15px;background:#f7fafb;border-bottom:1px solid #dfe7eb}.ic-us-public-head h2{margin:0;color:#073f56;font-size:15px}.ic-us-public-head span{font-size:8px;color:#74858e}.ic-us-public-kpis{display:grid;grid-template-columns:repeat(4,1fr);gap:8px;padding:12px 14px}.ic-us-kpi{border:1px solid #dce4e8;border-radius:6px;padding:10px;background:#fbfcfd}.ic-us-kpi small{display:block;font-size:7px;text-transform:uppercase;letter-spacing:.45px;color:#7c8b93}.ic-us-kpi strong{display:block;font-size:16px;color:#06445a;margin-top:4px}.ic-us-kpi span{display:block;font-size:7px;color:#7a858b;margin-top:3px}.ic-us-columns{display:grid;grid-template-columns:1fr 1fr;gap:10px;padding:0 14px 14px}.ic-us-card{border:1px solid #dce4e8;border-radius:6px;overflow:hidden}.ic-us-card-head{padding:8px 10px;background:#f4f8fa;border-bottom:1px solid #dfe6ea;color:#315563;font-size:8px;font-weight:750}.ic-us-detail{display:grid;grid-template-columns:120px 1fr;gap:4px 8px;padding:9px 10px;font-size:8px}.ic-us-detail dt{color:#829099}.ic-us-detail dd{margin:0;color:#314850;font-weight:650}.ic-us-awards{width:100%;border-collapse:collapse;font-size:8px}.ic-us-awards th{text-align:left;background:#f5f8fa;color:#78909b;font-size:7px;padding:7px 8px;border-bottom:1px solid #dce4e8}.ic-us-awards td{padding:7px 8px;border-bottom:1px solid #e8edef;vertical-align:top}.ic-us-awards tbody tr:nth-child(even){background:#fafcfd}.ic-us-awards tbody tr:hover{background:#edf7fa}.ic-us-money{font-weight:800;color:#075a70;white-space:nowrap}.ic-us-desc{max-width:330px;line-height:1.35}.ic-us-enrich{margin:12px 14px 14px;padding:13px;border:1px dashed #b9ccd5;border-radius:6px;background:#f8fbfc;display:flex;align-items:center;justify-content:space-between;gap:14px}.ic-us-enrich p{margin:0;color:#5f737d;font-size:9px;line-height:1.5}.ic-us-enrich button{border:1px solid #075a70;background:#075a70;color:#fff;border-radius:5px;padding:8px 10px;font-size:8px;font-weight:750;cursor:pointer;white-space:nowrap}.ic-us-enrich button:hover{background:#003f50}.ic-us-enrich button:disabled{opacity:.6;cursor:wait}.ic-us-note{padding:8px 14px;border-top:1px solid #e3e9ec;background:#fbfcfc;color:#7a878e;font-size:7px}
...
@media(max-width:900px){body[data-intell-profile] .ic-capability-strip{grid-template-columns:repeat(4,minmax(0,1fr))}body[data-intell-profile] .ic-capability:nth-child(6n){border-right:1px solid #e6ebee}body[data-intell-profile] .ic-capability:nth-child(4n){border-right:0}body[data-intell-profile] .ic-us-public-kpis{grid-template-columns:repeat(2,1fr)}body[data-intell-profile] .ic-us-columns{grid-template-columns:1fr}}
...
@media(max-width:600px){body[data-intell-profile] .profile-actions{width:100%}body[data-intell-profile] .profile-actions .small-btn,body[data-intell-profile] .ic-export-link{flex:1;justify-content:center}body[data-intell-profile] .ic-hs-tree{overflow-x:auto!important}body[data-intell-profile] .ic-hs-chapter{min-width:220px}body[data-intell-profile] .ic-us-enrich{align-items:flex-start;flex-direction:column}body[data-intell-profile] .ic-us-public-kpis{grid-template-columns:1fr 1fr}}
</style>
...
(()=>{
  ...
  function relabelProfile(company,market){
    const isUS=String(company.country||'').toUpperCase()==='US';
    document.querySelectorAll('.profile-meta div').forEach(el=>{if(/Canadian public importer relationships/i.test(el.textContent||''))el.textContent=(el.textContent||'').replace(/Canadian public importer relationships/i,'indexed business / trade relationships')});
    if(isUS){
      const snap=document.querySelector('.intel-summary');if(snap){const h=snap.querySelector('h2');if(h)h.textContent='U.S. Public-Data Snapshot';const p=snap.querySelector('p');if(p)p.textContent=`${company.name||'This company'} is matched as a U.S. ${(company.kind||'company').toLowerCase()}. IntellCluster combines canonical company evidence with available trade, federal-contract, fleet and compliance-oriented public records without treating missing shipment data as zero activity.`;snap.querySelectorAll('.coverage-cell span').forEach((s,i)=>s.textContent=['Product / HS codes','Origin markets','Matched datasets','Evidence records'][i]||s.textContent)}
      document.querySelectorAll('.section h2').forEach(h=>{if(/Canadian Product \/ HS Intelligence/i.test(h.textContent||''))h.textContent='U.S. Product / HS Intelligence'});
      document.querySelectorAll('.metric-label').forEach(h=>{if(/Importer Relationships/i.test(h.textContent||''))h.textContent='Indexed Relationships'});
    }
  }
  function renderUsPublic(company){
    if(String(company.country||'').toUpperCase()!=='US')return;
    document.getElementById('us-public-intelligence')?.remove();
    const enrichment=company.enrichment||{},f=enrichment.fmcsa||null,s=enrichment.usaspending||null;
    const sec=document.createElement('section');sec.className='ic-us-public profile-anchor';sec.id='us-public-intelligence';
    const fUnits=f?.power_units??'—',fDrivers=f?.total_drivers??'—',awardCount=s?.contract_awards_shown??'—',awardValue=s?.contract_award_value_shown;
    sec.innerHTML=`<div class="ic-us-public-head"><div><h2>U.S. Public Intelligence</h2><span>Free government-source enrichment cached by IntellCluster</span></div><span>${f?'FMCSA':''}${f&&s?' · ':''}${s?'USAspending.gov':''}</span></div><div class="ic-us-public-kpis"><div class="ic-us-kpi"><small>USDOT</small><strong>${esc(f?.usdot_number||'—')}</strong><span>${esc(f?.status||'FMCSA match not cached')}</span></div><div class="ic-us-kpi"><small>Power Units</small><strong>${esc(fUnits)}</strong><span>${f?'FMCSA fleet evidence':'Not cached'}</span></div><div class="ic-us-kpi"><small>Federal Awards Shown</small><strong>${esc(awardCount)}</strong><span>${s?'USAspending contracts':'Not cached'}</span></div><div class="ic-us-kpi"><small>Award Value Shown</small><strong>${awardValue!=null?money(awardValue):'—'}</strong><span>${s?.latest_modified?'Updated '+esc(String(s.latest_modified).slice(0,10)):'Not cached'}</span></div></div>`;
    const cols=document.createElement('div');cols.className='ic-us-columns';
    if(f){const card=document.createElement('div');card.className='ic-us-card';card.innerHTML=`<div class="ic-us-card-head">FMCSA Company Census / Fleet</div><dl class="ic-us-detail"><dt>Legal name</dt><dd>${esc(f.legal_name||company.name||'—')}</dd><dt>DBA</dt><dd>${esc(f.dba_name||'—')}</dd><dt>Status</dt><dd>${esc(f.status||f.status_code||'—')}</dd><dt>USDOT</dt><dd>${esc(f.usdot_number||'—')}</dd><dt>Power units</dt><dd>${esc(f.power_units??'—')}</dd><dt>Total drivers</dt><dd>${esc(f.total_drivers??'—')}</dd><dt>Carrier operation</dt><dd>${esc(f.carrier_operation||'—')}</dd><dt>Phone</dt><dd>${esc(f.phone||'—')}</dd><dt>MCS-150 date</dt><dd>${esc(f.mcs150_date||'—')}</dd></dl>`;cols.appendChild(card)}
    if(s){const card=document.createElement('div');card.className='ic-us-card';card.innerHTML=`<div class="ic-us-card-head">USAspending Federal Contract Evidence</div><dl class="ic-us-detail"><dt>Recipient</dt><dd>${esc(s.name||company.name||'—')}</dd><dt>UEI</dt><dd>${esc(s.uei||'—')}</dd><dt>DUNS</dt><dd>${esc(s.duns||'—')}</dd><dt>Trailing 12m recipient amount</dt><dd>${money(s.trailing_12m_recipient_amount||0)}</dd><dt>Award value shown</dt><dd>${money(s.contract_award_value_shown||0)}</dd><dt>Awarding agencies</dt><dd>${esc((s.awarding_agencies||[]).slice(0,4).join(' · ')||'—')}</dd><dt>NAICS</dt><dd>${esc((s.naics||[]).slice(0,6).join(' · ')||'—')}</dd><dt>PSC</dt><dd>${esc((s.psc||[]).slice(0,6).join(' · ')||'—')}</dd></dl>`;cols.appendChild(card)}
    if(cols.children.length)sec.appendChild(cols);
    if(s?.awards?.length){const wrap=document.createElement('div');wrap.style.overflow='auto';const rows=(s.awards||[]).slice(0,10).map(a=>`<tr><td>${esc(a.award_id||'—')}</td><td class="ic-us-money">${money(a.amount||0)}</td><td>${esc(a.awarding_agency||'—')}</td><td class="ic-us-desc">${esc(a.description||'—')}</td><td>${esc(a.start_date||'—')} → ${esc(a.end_date||'—')}</td></tr>`).join('');wrap.innerHTML=`<table class="ic-us-awards"><thead><tr><th>Award</th><th>Amount</th><th>Awarding Agency</th><th>Description</th><th>Period</th></tr></thead><tbody>${rows}</tbody></table>`;sec.appendChild(wrap)}
    if(!f||!s){const enrich=document.createElement('div');enrich.className='ic-us-enrich';enrich.innerHTML=`<p><strong>Expand this profile with free public data.</strong><br>${!f?'Check FMCSA fleet / USDOT evidence. ':''}${!s?'Check USAspending federal recipient and contract awards.':''} Results are normalized and cached; this does not call the paid ImportYeti API.</p><button type="button">Enrich Free U.S. Data</button>`;const btn=enrich.querySelector('button');btn.addEventListener('click',async()=>{btn.disabled=true;btn.textContent='Checking public sources…';try{const r=await fetch(`/api/intelligence/company/${encodeURIComponent(slug)}/enrich/us-public`,{method:'POST'}),p=await r.json();if(!r.ok)throw new Error(p.error||'Enrichment failed');renderUsPublic(p.company||company);relabelProfile(p.company||company,p.capabilities?.country||{});location.hash='us-public-intelligence'}catch(err){btn.disabled=false;btn.textContent='Try Again';enrich.querySelector('p').innerHTML=`<strong>Public enrichment could not complete.</strong><br>${esc(err.message||'Please try again later.')}`}});sec.appendChild(enrich)}
    const note=document.createElement('div');note.className='ic-us-note';note.textContent='This module uses free public U.S. government sources. A missing match means IntellCluster did not find a confident entity match; it does not prove the company has no fleet or federal contract activity.';sec.appendChild(note);
    const anchor=document.querySelector('.ic-capability-wrap')||metrics||document.querySelector('.profile-box');anchor?.insertAdjacentElement('afterend',sec);
    document.querySelectorAll('.ic-capability').forEach(cell=>{const label=cell.querySelector('.ic-capability-label')?.textContent?.trim();if(['Fleet','Contracts'].includes(label||'')){cell.style.cursor='pointer';cell.addEventListener('click',()=>sec.scrollIntoView({behavior:'smooth',block:'start'}));}});
  }
  fetch(`/api/intelligence/company/${encodeURIComponent(slug)}`).then(r=>r.ok?r.json():null).then(payload=>{
    ...
    relabelProfile(company,market);renderUsPublic(company);
  }).catch(()=>{});
