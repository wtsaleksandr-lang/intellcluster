...
def _same_place(
    ...
    if expected_state and state and state.strip().upper() != expected_state:
        return False
...
def _fmcsa_profile(company) -> dict[str, object]:
    return {
        ...
        "power_units": company.power_units,
        "total_drivers": company.total_drivers,
        "mcs150_date": company.mcs150_date,
...
async def _compliance(row: dict, enrichment: dict, lookup: dict[str, str]) -> None:
    ...
    if isinstance(enrichment.get("epa_echo"), dict):
        ...
        try:
            ...
            facilities = [
                ...
                and _same_place(state, city, facility.state, facility.city)
            ][:25]
            if facilities:
                with connect() as conn:
                    set_entity_enrichment(
                        ...
                        entity_id,
                        "epa_echo",
                        compact_echo_profile(facilities),
                    )
    ...
    if isinstance(enrichment.get("osha"), dict):
        ...
        try:
            inspections = await OSHAClient(timeout=12).search(
                name,
                state=state or None,
                limit=60,
            )
            ...
            if inspections:
                with connect() as conn:
                    set_entity_enrichment(
                        ...
                        entity_id,
                        "osha",
                        compact_osha_profile(inspections),
                    )
                lookup["osha"] = "matched"
            else:
...
async def intelligence_company_us_public_enrichment(slug: str) -> dict[str, object]:
    ...
    if str(row.get("country") or "").upper() != "US":
        raise HTTPException(
            status_code=400,
            detail="U.S. public enrichment applies to U.S. companies",
        )
    ...
    if isinstance(enrichment.get("fmcsa"), dict):
        lookup["fmcsa"] = "cached"
    else:
        try:
            ...
            if len(matches) == 1:
                with connect() as conn:
                    set_entity_enrichment(conn, entity_id, "fmcsa", _fmcsa_profile(matches[0]))
                lookup["fmcsa"] = "matched"
            else:
                lookup["fmcsa"] = "ambiguous" if matches else "no_confident_match"
        except (httpx.HTTPError, RuntimeError, ValueError):
            lookup["fmcsa"] = "unavailable"

    if isinstance(enrichment.get("usaspending"), dict):
        lookup["usaspending"] = "cached"
    else:
        try:
            ...
            if profile and _similar_name(name, profile_name) and place_ok:
                with connect() as conn:
                    set_entity_enrichment(conn, entity_id, "usaspending", profile)
                lookup["usaspending"] = "matched"
            else:
                lookup["usaspending"] = "no_confident_match"
        except (httpx.HTTPError, RuntimeError, ValueError):
            lookup["usaspending"] = "unavailable"
    ...
    return {
        "company": refreshed,
        "capabilities": profile_capabilities(refreshed),
        "lookup": lookup,
...
async def intelligence_company_compliance_enrichment(slug: str) -> dict[str, object]:
    ...
    if str(row.get("country") or "").upper() != "US":
        raise HTTPException(
            status_code=400,
            detail="Compliance enrichment currently applies to U.S. companies",
        )
