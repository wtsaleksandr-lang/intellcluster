...
async def sitemap_suppliers() -> Response:
    """Expose supplier profiles backed by cached relationship evidence."""
    with connect() as conn:
        rows = conn.execute(
            select(
    ...
    for row in rows:
        ...
        urls.append(
            _url_entry(
                f"/data/supplier/{quote(name, safe='')}",
...
async def sitemap_intelligence() -> Response:
    """Expose high-value analytical landing pages without search permutations."""
    with connect() as conn:
        ...
        origin_rows = conn.execute(
            select(
                importer_relationships.c.origin_country.label("origin"),
                func.count(importer_relationships.c.id).label("count"),
            )
        ...
        province_rows = conn.execute(
            select(
                entities.c.region.label("province"),
    ...
    for row in city_rows:
        ...
        if province and city:
            urls.append(
                _url_entry(
                    f"/data/location/{quote(province, safe='')}/{quote(city, safe='')}"
                )
            )
...
def _organization_schema(company: dict, canonical: str) -> dict[str, object]:
    ...
    if any(address_parts.values()):
        schema["address"] = {
            "@type": "PostalAddress",
            **{key: value for key, value in address_parts.items() if value},
        }
...
def _breadcrumb_schema(
    ...
    *,
    ...
    elif path == "/data/suppliers":
        crumbs.append(("Suppliers", path))
    else:
        match = re.fullmatch(r"/data/company/([^/]+)", path)
        if match:
            display = company_name or unquote(match.group(1)).replace("-", " ").title()
            crumbs.extend([("Companies", "/data/companies"), (display, path)])
        else:
            ...
            if match:
                ...
                if match:
                    code = match.group(1)
                    crumbs.extend(
                        [("HS Code Intelligence", "/data"), (f"HS {code}", path)]
                    )
                else:
                    ...
                    if match:
                        ...
                        crumbs.append(
                            (
                                province,
                                f"/data/location/{quote(province, safe='')}",
                            )
                        )
                        if city:
...
def _rendered_meta(text: str) -> tuple[str, str]:
    ...
    title = (
        ...
        if title_match
        else "IntellCluster Data"
    )
    description = (
        re.sub(r"\s+", " ", desc_match.group(1)).strip()
        if desc_match
        else "Public business intelligence and company data."
    )
...
def install_seo_middleware(app) -> None:
    ...
    async def intellcluster_seo_metadata(request: Request, call_next):
        ...
        canonical = _base_url() + clean_path
        noindex = (
            response.status_code >= 400
            or path.startswith("/data/search")
            or path.startswith("/data/suggest")
            or (
                clean_path in {"/data/companies", "/data/suppliers"}
                and bool(request.query_params)
            )
        )
        robots = (
            "noindex,follow"
        ...
        if company_match and response.status_code == 200:
            ...
            try:
                ...
                if company:
                    ...
                    head_bits.append(
                        f'<script type="application/ld+json">{safe_schema}</script>'
                    )
            except Exception:
                pass
