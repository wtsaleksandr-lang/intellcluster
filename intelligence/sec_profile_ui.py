...
@media(max-width:600px){body[data-intell-profile] .ic-sec-head{flex-direction:column}body[data-intell-profile] .ic-sec-grid{grid-template-columns:repeat(2,1fr)}body[data-intell-profile] .ic-sec-empty{align-items:flex-start;flex-direction:column}}
</style>
...
(() => {
  ...
  const mount=(company)=>{
    ...
    const edgar=company?.enrichment?.sec_edgar||null;
    const section=document.createElement('section');section.id='sec-edgar-intelligence';section.className='ic-sec profile-anchor';
    section.innerHTML=`<div class="ic-sec-head"><div><div class="ic-sec-kicker">U.S. PUBLIC FILINGS</div><h2>SEC EDGAR Intelligence</h2><p>Public-company filing evidence cached from SEC EDGAR. Normal profile views never call the SEC network.</p></div>${edgar?'<span class="ic-sec-source"><i></i> SEC EDGAR cached</span>':''}</div>`;
    if(edgar){
      ...
      const empty=document.createElement('div');empty.className='ic-sec-empty';empty.innerHTML='<div><strong>SEC filing evidence has not been cached for this profile.</strong>The free check uses SEC ticker/CIK associations and EDGAR submissions. A missing match does not prove the company has no SEC filings.</div><button type="button">Check SEC EDGAR</button>';const button=empty.querySelector('button');button.addEventListener('click',async()=>{button.disabled=true;button.textContent='Checking SEC…';try{const response=await fetch(`/api/intelligence/company/${encodeURIComponent(slug)}/enrich/sec-edgar`,{method:'POST'});const payload=await response.json();if(!response.ok)throw new Error(payload.detail||'SEC lookup failed');const status=payload.lookup?.sec_edgar;if(status==='matched'||status==='cached'){button.textContent='SEC data cached';mount(payload.company||company);return;}button.disabled=false;button.textContent='Check Again';const strong=empty.querySelector('strong');if(strong)strong.textContent=status==='ambiguous'?'Multiple possible SEC matches — no automatic link was saved.':status==='no_confident_match'?'No confident ticker-associated SEC match found.':'SEC EDGAR is temporarily unavailable.';}catch(error){button.disabled=false;button.textContent='Try Again';const strong=empty.querySelector('strong');if(strong)strong.textContent=error.message||'SEC lookup could not complete.';}});section.appendChild(empty);
    }
