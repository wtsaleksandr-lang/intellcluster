...
def _find_entity(conn: Connection, record: SourceRecord) -> int | None:
    ...
    for candidate in candidates:
        match = score_company_match(record, _candidate_record(dict(candidate)))
        if match.is_likely_match:
            scored.append((match.score, int(candidate["id"])))
    if not scored:
        return None
