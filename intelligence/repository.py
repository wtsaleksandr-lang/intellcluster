...
def _find_entity(conn: Connection, record: SourceRecord) -> int | None:
    ...
    for candidate in candidates:
        ...
        postal_ok = not record.postal_code or not candidate["postal_code"] or record.postal_code.replace(" ", "").casefold() == candidate["postal_code"].replace(" ", "").casefold()
...
def _unique_slug(conn: Connection, record: SourceRecord) -> str:
    suffix = None
    if record.attributes.get("corporation_number"):
        suffix = str(record.attributes["corporation_number"])
...
def search_entities(
    ...
    *,
    ...
    if q:
        term = f"%{q.casefold()}%"
        rel_text = exists(select(importer_relationships.c.id).where(
            and_(importer_relationships.c.entity_id == entities.c.id, func.lower(func.coalesce(importer_relationships.c.product_description, "")).like(term))
        ))
    ...
    if origin:
        conditions.append(exists(select(importer_relationships.c.id).where(
            and_(importer_relationships.c.entity_id == entities.c.id, func.lower(importer_relationships.c.origin_country).like(f"%{origin.casefold()}%"))
        )))
    if hs:
        digits = "".join(ch for ch in hs if ch.isdigit())
        if digits:
            conditions.append(exists(select(importer_relationships.c.id).where(
                and_(importer_relationships.c.entity_id == entities.c.id, or_(importer_relationships.c.hs6.like(f"{digits}%"), importer_relationships.c.hs10.like(f"{digits}%")))
            )))
    if conditions:
...
def _decorate_company(conn: Connection, company: dict[str, Any], full: bool = False) -> dict[str, Any]:
    ...
    relationships = conn.execute(
        select(importer_relationships).where(importer_relationships.c.entity_id == entity_id).limit(500 if full else 100)
    ).mappings().all()
    ...
    source_count = conn.execute(select(func.count(func.distinct(source_records.c.source))).where(source_records.c.entity_id == entity_id)).scalar_one()
    company.update(
